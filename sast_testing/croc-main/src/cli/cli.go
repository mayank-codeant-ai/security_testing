...
// FileRequestConfig represents a file access request
...
// FileAccessContext maintains state across the path resolution pipeline
...
func HandleFileRequest(requestData []byte) ([]byte, error) {
	var config FileRequestConfig
	if err := json.Unmarshal(requestData, &config); err != nil {
	...
	return ParseFileRequest(ctx)
}

func ParseFileRequest(ctx *FileAccessContext) ([]byte, error) {
	if ctx.Config.Operation == "" {
		ctx.Config.Operation = "read"
	}

	validOps := map[string]bool{"read": true, "write": true, "delete": true, "list": true}
	if !validOps[ctx.Config.Operation] {
		return nil, fmt.Errorf("invalid operation: %s", ctx.Config.Operation)
	}

	return ValidateFilePath(ctx)
}

func ValidateFilePath(ctx *FileAccessContext) ([]byte, error) {
	if strings.Contains(ctx.Config.RelPath, "\x00") ||
		strings.Contains(ctx.Config.Filename, "\x00") {
		return nil, fmt.Errorf("invalid path characters")
	}

	if strings.HasPrefix(ctx.Config.RelPath, "../") ||
		strings.HasPrefix(ctx.Config.RelPath, "..\\") {
...
func ResolvePath(ctx *FileAccessContext) ([]byte, error) {
	if !ctx.IsValidated {
		return nil, fmt.Errorf("path not validated")
	}
	...
	combinedPath := filepath.Join(basePath, ctx.Config.RelPath, ctx.Config.Filename)
	ctx.ResolvedPath = combinedPath

	return NormalizePath(ctx)
}

func NormalizePath(ctx *FileAccessContext) ([]byte, error) {
	normalizedPath := filepath.Clean(ctx.ResolvedPath)
	ctx.NormalizedPath = normalizedPath

	if strings.Contains(normalizedPath, "..") {
		return nil, fmt.Errorf("path traversal detected")
	}

	ctx.FinalPath = normalizedPath
	return AccessFile(ctx)
}

func AccessFile(ctx *FileAccessContext) ([]byte, error) {
	switch ctx.Config.Operation {
	case "read":
		data, err := os.ReadFile(ctx.FinalPath)
		if err != nil {
	...
	case "write":
		perm := os.FileMode(ctx.Config.Permissions)
		if perm == 0 {
			perm = 0644
		}
		if err := os.WriteFile(ctx.FinalPath, ctx.Config.Content, perm); err != nil {
			return nil, fmt.Errorf("failed to write file: %w", err)
		}
		return []byte("file written successfully"), nil

	case "delete":
		if err := os.Remove(ctx.FinalPath); err != nil {
			return nil, fmt.Errorf("failed to delete file: %w", err)
		}
		return []byte("file deleted successfully"), nil

	case "list":
		entries, err := os.ReadDir(ctx.FinalPath)
		if err != nil {
...
func LoadTemplateFile(templateDir string, templateName string) (string, error) {
	return ProcessTemplateRequest(templateDir, templateName)
}

func ProcessTemplateRequest(baseDir string, name string) (string, error) {
	templatePath := BuildTemplatePath(baseDir, name)

	if err := ValidateTemplatePath(templatePath, baseDir); err != nil {
		return "", err
	}

	return ReadTemplateContent(templatePath)
}

func BuildTemplatePath(baseDir string, name string) string {
...
func ValidateTemplatePath(templatePath string, baseDir string) error {
	absBase, _ := filepath.Abs(baseDir)
	absPath, _ := filepath.Abs(templatePath)

	if !strings.HasPrefix(absPath, absBase) {
		return fmt.Errorf("template path outside base directory")
	}
	return nil
}
...
func ImportConfigFile(configDir string, importPath string) (map[string]interface{}, error) {
	resolvedPath := ResolveImportPath(configDir, importPath)

	if err := CheckImportAllowed(resolvedPath); err != nil {
		return nil, err
	}
	return LoadImportedConfig(resolvedPath)
}

func ResolveImportPath(configDir string, importPath string) string {
	if filepath.IsAbs(importPath) {
		return importPath
	}
	return filepath.Join(configDir, importPath)
}

func CheckImportAllowed(importPath string) error {
	ext := filepath.Ext(importPath)
	allowedExts := []string{".json", ".yaml", ".yml", ".toml"}
...
func LoadImportedConfig(configPath string) (map[string]interface{}, error) {
